...
function PatentSummarizer() {
    ...
    const handleInputChange = (event) => {
        ...
        setInputWordLength(event.target.value.split(/\s+/).length);
    }
    ...
    return (
        <div className="container">
            ...
            <div className="input-box">
                <div className="label">Put you Patent content below: <div>Words: {inputWordLength}</div></div>
                <br></br>
                <TextareaAutosize minRows={15}  value={inputText} onChange={handleInputChange} className = "inputArea">
                </TextareaAutosize>
                <div className="error-messages">
                {inputWordLength < 10  ?  <div>Patent text is too small</div> : ''}
                </div>
                ...
                abstractGenerated !== '' && !showSpinner
                    ? (
                        <div className="output-box">
                            <div className="label">Summary: <div>Words: {abstractGenerated.split(" ").length}</div></div>
                            <div>
...

        {
                ...
                <div class="loader">
                    <CircularProgress />
                    <label>Patience, your patent is being summarized!</label>
                </div>
            }
        </div>
    );
